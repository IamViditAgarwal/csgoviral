	...
	//	$url = 'http://127.0.0.1:8888?steamid=76561198306639779';
		...
		if(strcmp($contents, "Error")==0)
		...
		else 
			{
					...
					$inventoryTable= '<div align= "center"  style="text-align:center ;">
					...
					foreach ($json_inventory as $key => $inventoryItem) {
								...
								//{		
										...
										foreach ($inventoryItem as $itemName => $itemValue) {
												...
												{	
													...
													else if( (strpos($itemValue, "CZ75-Auto",0 ) !== false ) || (strpos($itemValue, "USP-S",0 ) !== false ) || (strpos($itemValue, "Desert Eagle",0 ) !== false ) ||(strpos($itemValue, "Tec-9",0 ) !== false ) ||(strpos($itemValue, "Dual Berettas" , 0) !== false ) || (strpos($itemValue, "Glock-18" , 0) !== false ) || (strpos($itemValue, "Five-SeveN" , 0) !== false ) || (strpos($itemValue, "P2000" , 0) !== false ) || (strpos($itemValue, "P250" , 0) !== false ) || (strpos($itemValue, "R8 Revolver" , 0) !== false ))
													{
														//means contain item
						...
						if ($fr->num_rows > 0) {
						    ...
						    $fs="-";
													...
													$inventoryTable.='<li style="border:2px solid rgba(0,0,0,0) ; position:relative; padding-bottom:5px; padding-right:5px;">';
													...
													$inventoryTable.='<div data-tradeable-type="'.$trade_type.'" id="inventory_item_number_'.$col_count.'"  '.$cc.'       onclick="addItem(\''.$next_item.'\',\''.$weapon_name[0].'\',\''.$skin_name.'\',\''.$exterior.'\',\''.$fs.'\',\''.$rarity.'\',\'inventory_item_number_'.$col_count.'\', \''.$tt.'\' )"   >' ;
					...
					if($col_count==0)
					{
						...
						echo '<div type="error" class="fav_font" style="width:50%; height:auto;  margin:0 auto; left:0; right:0; font-size : 18px; margin-top:10px; padding-top:5px; padding-bottom:10px; color :#7b7b7b;">No Pistols Found!</div>';
						echo "</div>" ;
					}
