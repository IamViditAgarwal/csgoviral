	...
	//	$url = 'http://127.0.0.1:8888?steamid=76561198306639779';
		...
		if(strcmp($contents, "Error")==0)
		...
		else 
			{
								...
								//{		
										...
										foreach ($inventoryItem as $itemName => $itemValue) {
												...
												{	
													...
													else if( (strpos($itemValue, "Bayonet",0 ) !== false ) || (strpos($itemValue, "Karambit",0 ) !== false ) || (strpos($itemValue, "Bowie Knife",0 ) !== false ) ||(strpos($itemValue, "Butterfly Knife",0 ) !== false ) ||(strpos($itemValue, "Falchion Knife",0 ) !== false ) ||(strpos($itemValue, "Flip Knife",0 ) !== false ) ||(strpos($itemValue, "Gut Knife",0 ) !== false ) ||(strpos($itemValue, "Huntsman Knife",0 ) !== false ) ||(strpos($itemValue, "M9 Bayonet",0 ) !== false ) ||(strpos($itemValue, "Shadow Daggers",0 ) !== false ))
													{
														//means contain item
					...
					if($col_count==0)
					{
						echo "<div class='fav_font' id='no_element' style='width :40%; height :auto;  margin:0 auto; left:0; right:0;  text-align: center;'>" ;
						echo '<div type="error" class="fav_font" style="width:50%; height:auto;  margin:0 auto; left:0; right:0; font-size : 18px; margin-top:10px; padding-top:5px; padding-bottom:10px; color :#7b7b7b;">No Knives Found!</div>';
						echo "</div>" ;
					}
