	...
	$url = 'http://127.0.0.1:8888?steamid='.$_SESSION['steamid'].'';
		...
		if(strcmp($contents, "Error")==0)
		...
		else 
			{
								...
								//{		
										...
										foreach ($inventoryItem as $itemName => $itemValue) {
												...
												{	
													...
													else if( (strpos($itemValue, "MAG-7",0 ) !== false ) || (strpos($itemValue, "Nova",0 ) !== false ) || (strpos($itemValue, "Sawed-Off",0 ) !== false ) ||(strpos($itemValue, "XM1014",0 ) !== false ) ||(strpos($itemValue, "M249" , 0) !== false ) || (strpos($itemValue, "Negev" , 0) !== false ) )
													{
														//means contain item
					...
					if($col_count==0)
					{
						echo "<div id='no_element' class='fav_font' style='width :50%; height :auto;  margin:0 auto; left:0; right:0;  text-align: center;'>" ;
						echo '<div type="error" class="fav_font" style="width:50%; height:auto;  margin:0 auto; left:0; right:0; font-size : 18px; margin-top:10px; padding-top:5px; padding-bottom:10px; color :#7b7b7b;">No Heavy Gun Found!</div>';
						echo "</div>" ;
					}
