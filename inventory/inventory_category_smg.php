		...
		if(strcmp($contents, "Error")==0)
		...
		else 
			{
					...
					foreach ($json_inventory as $key => $inventoryItem) {
										...
										foreach ($inventoryItem as $itemName => $itemValue) {
												...
												{	
													...
													else if( (strpos($itemValue, "MAC-10",0 ) !== false ) || (strpos($itemValue, "MP7",0 ) !== false ) || (strpos($itemValue, "MP9",0 ) !== false ) ||(strpos($itemValue, "PP-Bizon",0 ) !== false ) ||(strpos($itemValue, "P90" , 0) !== false ) || (strpos($itemValue, "UMP-45" , 0) !== false ) )
													{
														//means contain item
					...
					if($col_count==0)
					{
						...
						echo '<div type="error" class="fav_font" style="width:50%; height:auto;  margin:0 auto; left:0; right:0; font-size : 18px; margin-top:10px; padding-top:5px; padding-bottom:10px; color :#7b7b7b;">No SMGs Found!</div>';
						echo "</div>" ;
					}
